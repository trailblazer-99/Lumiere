...
namespace LumiereMediaPlayer.Pages
{
    public sealed partial class StreamingMoviesPage : Page
    {
        ...
        public Visibility GetDiscoverEmptyState(bool isLoading, bool hasError) =>
            (ViewModel.Movies != null && ViewModel.Movies.Count == 0 && !isLoading && !hasError) ? Visibility.Visible : Visibility.Collapsed;
        ...
        private void OnSearchBoxQuerySubmitted(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs? args)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[StreamingMoviesPage] OnSearchBoxQuerySubmitted error: {ex.Message}");
            }
        }
        ...
        private void OnPageLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Failed to animate StreamingMoviesPage entrance: {ex.Message}");
                PageContent.Opacity = 1.0;
            }
        }
        ...
        private async System.Threading.Tasks.Task LoadTrendingAsync()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"LoadTrendingAsync movies error: {ex.Message}");
            }
        }
        ...
        private void OnContextAddWatchlistClick(object sender, RoutedEventArgs e)
        {
            if (sender is FrameworkElement fe)
            {
                ...
                {
                    AppServices.StreamingLibrary.AddItem(new SavedStreamingItem { Id = $"tmdb_movie-{tm.Id}", Title = tm.DisplayTitle ?? "", Subtitle = $"({tm.DisplayYear})", PosterUrl = tm.PosterUrl, Type = Services.Streaming.StreamingItemType.Movie, Watchlist = "Watchlist" });
                }
            }
        }
